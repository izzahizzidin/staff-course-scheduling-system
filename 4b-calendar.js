var cal = {
  ...
  ajax: function(data, cb) {
    ...
    xhr.open("POST", "3-cal-ajax.php", true);
  ...
  init: () => {
    ...
    cal.hForm.onsubmit = () => cal.save().then(result => {
      if (result) {
        cal.hFormWrap.close();
      }
    });
  ...
  show: id => {
    if (id) {
      ...
      cal.hfParticipants.value = cal.events[id]["par"];
  ...
  save: () => {
    ...
    var data = {
      ...
      bg: cal.hfBG.value,
      coursecategory: cal.hfCourseCategory.value,
      division: cal.hfDivision.value,
      venue: cal.hfVenue.value,
      duration: cal.hfDuration.value,
      categoryarea: cal.hfCategoryArea.value,
      participants: $("#evtParticipants").val() // retrieve the value of the select2 element
    };
    if (cal.hfID.value != "") {
